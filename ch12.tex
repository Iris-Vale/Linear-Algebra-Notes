...
\textbf{6.~~$f(x)$是有穷区间$[0,l]$上的有界可积函数：}做奇延拓或偶延拓，再做周期延拓，然后计算傅里叶级数.\vspace{1em}
...
以向量运算的性质作为“模板”，可以公理化地定义加法和数乘运算，从而构建出一般的线性空间（对公理化定义不感兴趣者可跳过，不影响理解）：\vspace{1em}
...
(8)~\textbf{数乘分配律II：}\textit{$(\alpha + \beta)\bm{x} = \alpha \bm{x} + \beta \bm{x}$，$\forall \alpha,\beta \in \mathbb{R}$，$\bm{x} \in V$.}\vspace{1em}
...
对于第二个问题，还需要用以下定理打个补丁：\vspace{1em}
